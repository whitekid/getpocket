...
func TestPocket(t *testing.T) {
	...
	{
		resp, err := client.Get().Since(now).Do(ctx)
		require.NoError(t, err)
		require.Equal(t, 0, len(resp.List))
	}
	...
	var itemId string
	{
		...
		resp, err := client.Add(url).Do(ctx)
		require.NoError(t, err)
		require.Equal(t, url, resp.Items.ResolvedURL)
		itemId = resp.Items.ItemID
	...
	// 1개 추가되었으니, 리스트에 하나 보임
	{
		resp, err := client.Get().Since(now).Do(ctx)
		require.NoError(t, err)
		require.Equal(t, 1, len(resp.List))
	}
...
func TestUnmarshalArticle(t *testing.T) {
	...
	tests := [...]struct {
		...
		{"without_video", args{"article_with_image.json"}, "3575623899"},
	}
...
func TestUnmarshalAddResponse(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			require.Equal(t, tt.wantVideos, len(resp.Items.Videos))
			require.Equal(t, tt.wantImages, len(resp.Items.Images))
		})
	}
